...
namespace TechnologerMod.Content.Projectiles
{
	...
	public class PhoenixSlash : ModProjectile
	{
		...
		public override void AI() {
			...
			if (Main.rand.NextFloat() * 2f < Projectile.Opacity) {
				// Original Excalibur color: Color.Red, Color.Orange
			...
			if (Main.rand.NextFloat() * 1.5f < Projectile.Opacity) {
				// Original Excalibur color: Color.Orange
		...
		public override bool? Colliding(Rectangle projHitbox, Rectangle targetHitbox) {
			...
			if (backOfTheSwing > 0f) {
				...
				// Uncomment this line for a visual representation of the cone. The dusts are not perfect, but it gives a general idea.
				// Dust.NewDustPerfect(Projectile.Center + coneRotation2.ToRotationVector2() * coneLength, DustID.Enchanted_Red, Vector2.Zero);
